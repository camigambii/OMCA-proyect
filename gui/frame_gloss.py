...
class FrameGloss(ctk.CTkFrame):
    def __init__(self, parent, app):
        ...
        # =========================================================
        # 1. CONEXIÓN AL BACKEND REAL (ÁRBOL BST)
        # =========================================================
        inventario_completo = config.arbol_inventario.obtener_lista_inorden()
        
        # Filtramos solo los que son "Gloss"
        self._todos = []
    ...
    def _construir(self):
        ctk.CTkLabel(
            self, text="G L O S S  &  L A B I A L E S", font=("Georgia", 20, "bold"), text_color="#1A1A1A"
        ).pack(pady=(22, 4))

        ctk.CTkLabel(
            self, text="Descubre el tono perfecto para ti", font=("Arimo", 12), text_color="#888888"
        ).pack(pady=(0, 14))
        ...
        self.entry_buscar = ctk.CTkEntry(
            barra, placeholder_text="🔍  Buscar producto...", font=("Arimo", 13), width=240, height=36, corner_radius=18, fg_color="#FFFFFF"
        )
        self.entry_buscar.pack(side="left", padx=(0, 12))
        self.entry_buscar.bind("<KeyRelease>", self._on_buscar)

        ctk.CTkLabel(barra, text="Ordenar:", font=("Arimo", 12), text_color="#666666").pack(side="left", padx=(0, 6))

        self.opt_orden = ctk.CTkOptionMenu(
            barra, values=["Precio ↑", "Precio ↓", "A → Z", "Z → A"], font=("Arimo", 12), width=130, height=36, fg_color="#FFFFFF", text_color="#333333", command=self._on_ordenar
        )
        ...
        self.panel_catalogo = ctk.CTkScrollableFrame(cuerpo, fg_color="#FAFAFA", corner_radius=0)
    ...
    def _construir_carrito(self, parent):
        panel = ctk.CTkFrame(parent, fg_color="#FFFFFF", corner_radius=12, border_width=1, border_color="#EBEBEB", width=240)
        panel.pack(side="right", fill="y", padx=(0, 0))
        panel.pack_propagate(False)

        ctk.CTkLabel(panel, text="🛒  Mi Carrito", font=("Georgia", 15, "bold"), text_color="#1A1A1A").pack(pady=(18, 6), padx=16)
        ctk.CTkFrame(panel, fg_color="#EBEBEB", height=1).pack(fill="x", padx=16)

        self.scroll_carrito = ctk.CTkScrollableFrame(panel, fg_color="transparent", height=280)
        self.scroll_carrito.pack(fill="x", padx=10, pady=8)

        ctk.CTkFrame(panel, fg_color="#EBEBEB", height=1).pack(fill="x", padx=16)

        self.lbl_total = ctk.CTkLabel(panel, text="Total:  $0.00", font=("Arimo", 14, "bold"), text_color="#1A1A1A")
        self.lbl_total.pack(pady=(10, 6), padx=16)

        ctk.CTkButton(
            panel, text="Confirmar Compra", fg_color="#dd838e", hover_color="#c96f7b", font=("Arimo", 13, "bold"), height=38, command=self._confirmar_compra
        ).pack(fill="x", padx=16, pady=(0, 8))

        ctk.CTkButton(
            panel, text="Vaciar carrito", fg_color="transparent", text_color="#999999", hover_color="#F5F5F5", command=self._vaciar_carrito
        ).pack(fill="x", padx=16, pady=(0, 14))

    # ════════════════════════════════════════════════════════════════════
    # CATÁLOGO CON OBJETOS REALES
    # ════════════════════════════════════════════════════════════════════
    def _poblar_catalogo(self, productos: list):
        ...
        if not productos:
            ctk.CTkLabel(self.panel_catalogo, text="No hay productos en esta categoría.", font=("Arimo", 14), text_color="#AAAAAA").grid(row=0, column=0, columnspan=COLS, pady=40)
            return
    ...
    def _crear_tarjeta(self, prod, fila: int, col: int):
        tarjeta = ctk.CTkFrame(self.panel_catalogo, fg_color="#FFFFFF", corner_radius=14, border_width=1, border_color="#EBEBEB", width=200, height=280)
        ...
        # Textos
        ctk.CTkLabel(tarjeta, text=prod.nombre, font=("Georgia", 12, "bold"), text_color="#1A1A1A", wraplength=170).pack(padx=10)
        ctk.CTkLabel(tarjeta, text=f"${prod.precio:.2f}", font=("Arimo", 13), text_color="#c14b4f").pack(pady=(2, 0))

        color_stock = "#27AE60" if prod.stock > 0 else "#E74C3C"
        txt_stock   = f"Stock: {prod.stock}" if prod.stock > 0 else "Agotado"
        ctk.CTkLabel(tarjeta, text=txt_stock, font=("Arimo", 10), text_color=color_stock).pack()
        ...
        ctk.CTkButton(
            btns, text="+ Carrito", fg_color="#1A1A1A", font=("Arimo", 11, "bold"), height=30, corner_radius=8,
            state="normal" if prod.stock > 0 else "disabled", command=lambda p=prod: self._agregar_carrito(p)
        ).pack(side="left", expand=True, fill="x", padx=(0, 4))

        # Botón Tester
        ctk.CTkButton(
            btns, text="👄", fg_color="#FFF0F0", text_color="#dd838e", font=("Arimo", 14), width=32, height=30, corner_radius=8,
    ...
    # ════════════════════════════════════════════════════════════════════
    # BÚSQUEDA Y ORDENAMIENTO (Adaptado a Objetos)
    # ════════════════════════════════════════════════════════════════════
    ...
    def _partition(self, lista, bajo, alto, clave, ascendente):
        pivote = getattr(lista[alto], clave)
        i = bajo - 1
        for j in range(bajo, alto):
            val_j = getattr(lista[j], clave)
            cond = val_j <= pivote if ascendente else val_j >= pivote
            if cond:
                i += 1
    ...
    def _merge(self, izq, der, clave, ascendente):
        ...
        while i < len(izq) and j < len(der):
            val_i = getattr(izq[i], clave)
            val_j = getattr(der[j], clave)
            cond = val_i <= val_j if ascendente else val_i >= val_j
            if cond:
    ...
    # ════════════════════════════════════════════════════════════════════
    # CARRITO (Adaptado a Objetos)
    # ════════════════════════════════════════════════════════════════════
    def _agregar_carrito(self, prod):
        self._carrito.append(prod)
    ...
    def _confirmar_compra(self):
        ...
        ventana.grab_set()
        ctk.CTkLabel(ventana, text="✓  Orden Confirmada", font=("Georgia", 18, "bold"), text_color="#27AE60").pack(pady=(36, 8))
        ctk.CTkLabel(ventana, text="Se ha enviado un correo con el detalle\nde tu compra.", font=("Arimo", 13)).pack()
    ...
    def _refrescar_carrito(self):
        ...
        for item in self._carrito:
            ...
            ctk.CTkLabel(fila, text=item.nombre, font=("Arimo", 11), wraplength=130, anchor="w").pack(side="left", fill="x", expand=True)
            ctk.CTkLabel(fila, text=f"${item.precio:.2f}", font=("Arimo", 11, "bold"), text_color="#c14b4f").pack(side="right")

        total = sum(item.precio for item in self._carrito)
    ...
    def _probar_en_tester(self, prod):
        # Aseguramos que no truene si el color viene vacío desde el CSV
