...
class FrameBronzer(ctk.CTkFrame):
    def __init__(self, parent, app):
        ...
        for p in inventario_completo:
            cat_limpia = p.categoria.strip().lower()
            if "bronzer" in cat_limpia or "bronceador" in cat_limpia:
    ...
    def _construir(self):
        ctk.CTkLabel(
            self,
            text="B R O N Z E R  &  C O N T O U R",
        ...
        ctk.CTkLabel(
            self,
            text="Define y broncea tu rostro con un acabado natural",
            font=("Arimo", 12),
            text_color="#888888"
        ).pack(pady=(0, 14))
        ...
        self.entry_buscar = ctk.CTkEntry(
            barra,
            placeholder_text="🔍  Buscar bronzer...",
            font=("Arimo", 13),
            width=240,
            height=36,
            corner_radius=18,
    ...
    def _poblar_catalogo(self, productos: list):
        ...
        if not productos:
            ctk.CTkLabel(
                self.panel_catalogo,
                text="No hay bronzers en esta categoría.",
                font=("Arimo", 14),
                text_color="#AAAAAA"
    ...
    def _crear_tarjeta(self, prod, fila: int, col: int):
        ...
        # Validación del Tester: Evitar colores negros puros (0,0,0)
        if not (prod.color_r == 0 and prod.color_g == 0 and prod.color_b == 0):
            ...
            ctk.CTkButton(
                ...
                height=30,
                corner_radius=8,
                command=lambda r=val_r, g=val_g, b=val_b: self._probar_en_tester(r, g, b, prod.nombre)
            ).pack(side="right")
    ...
    # TESTER
    # ════════════════════════════════════════════════════════════════════
    def _probar_en_tester(self, r, g, b, nombre):
        # Actualizado para recibir las variables correctamente de tu clase Producto
        print(f"[Tester] Probando Bronzer: {nombre} con Color RGB: ({r},{g},{b})")
